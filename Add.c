...
void addScreen() {
    ...
    WINDOW *addwin = newwin(LINES - 2, COLS - 2, 1, 1);

    box(addwin, 0, 0);
    mvwprintw(addwin, 1, (COLS - strlen("Add A New Contact")) / 2, "Add A New Contact");
...
static void inputName(WINDOW *addwin, t_data *newContact) {
    ...
    if (strchr(newContact->name, ':')) {    // 이름필드에 구분자를 넣었다면 에러를 출력시키고 다시 입력받음
        ...
        eraseCursor(addwin, 1 + add_y / 8, add_x / 8 + 7, strlen(newContact->name)); // 입력한 이름을 지움
...
static void inputPhone(WINDOW *addwin, t_data *newContact) {
    ...
    if (!isValidPhone(newContact->phone)) { 
        errorScreen(addwin, "Can't input invalid phone number"); // 전화번호가 유효하지 않으면 오류를 출력시키고 다시 입력받음
        eraseCursor(addwin, 1 + add_y / 4, add_x / 8 + 8, strlen(newContact->phone)); // 입력한 전화번호를 지움
    ...
    if (checkPhoneExists(newContact->phone)) {  // 전화번호 필드가 중복되면 오류를 출력시키고 다시 입력받음
        errorScreen(addwin, "Can't register duplicated phone number");
        eraseCursor(addwin, 1 + add_y / 4, add_x / 8 + 8, strlen(newContact->phone)); // 입력한 전화번호를 지움
        return inputPhone(addwin, newContact);
    }
}
...
static void inputMemo(WINDOW *addwin, t_data *newContact) {
    ...
    if (strchr(newContact->memo, ':')) {    // 이름필드에 구분자를 넣었다면 에러를 출력시키고 다시 입력받음
        ...
        eraseCursor(addwin, 1 + add_y / 2, add_x / 8 + 7, strlen(newContact->memo)); // 입력한 메모를 지움
